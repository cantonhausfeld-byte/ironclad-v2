...
class BronzeWriter(_BaseWriter):
    ...
    def write_ngs_receiving(self, df: pd.DataFrame) -> int:
        df = df.copy()
        df["_ingest_ts"] = _now()
        return self._upsert(df, "bronze.ngs_receiving", ["season", "week", "player_id"])

    def write_ngs_passing(self, df: pd.DataFrame) -> int:
        df = df.copy()
        df["_ingest_ts"] = _now()
    ...
    def write_ngs_rushing(self, df: pd.DataFrame) -> int:
        df = df.copy()
        df["_ingest_ts"] = _now()
        return self._replace(df, "bronze.ngs_rushing", ["season", "week", "player_id"])
